...
class my_auction_page : AppCompatActivity() {


    object Global2 {
        var liked_list = mutableListOf<posts>()
    }
    lateinit var post_adapter2: add_post
    lateinit var rv2:RecyclerView
    lateinit var nothingPic1:ImageView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        nothingPic1 =findViewById<ImageView>(R.id.nothing_pic1)
        ...
        val nestedScrollView=findViewById<NestedScrollView>(R.id.nestedScrollView)
            ...
            nothingPic1.visibility= View.INVISIBLE
            post_adapter2 = add_post(this, Global2.liked_list)
            rv2.adapter = post_adapter2
            rv2.layoutManager = LinearLayoutManager(this)

            // Todo posts on click listener
            post_adapter2.onItemClick={
    ...
    //Todo function to get all the cars the current user has bid on
    @SuppressLint("NotifyDataSetChanged", "SuspiciousIndentation")
    fun EventChangeListener() {
        val db = FirebaseFirestore.getInstance()
        val uid = Firebase.auth.currentUser
        db.collection("users_auction").whereEqualTo("user_id" , uid?.email.toString()).whereEqualTo("user","auction")
            .get()
            .addOnSuccessListener { result ->
                Global2.liked_list.clear()
                if (result.isEmpty){
                    rv2.visibility = View.INVISIBLE
                    nothingPic1.visibility = View.VISIBLE
                }
                else
                for (document in result ) {
                    if (document != null) {
                        ...
                        Global2.liked_list.add(ob)
                    }
                }
                    post_adapter2.notifyDataSetChanged()
            }
